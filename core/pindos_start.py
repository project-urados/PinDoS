...
parser = argparse.ArgumentParser(
    description="Launch or terminate Pindos background processes."
)
...
parser.add_argument(
    "--count", type=int, default=1, help="Number of packets per process"
)
parser.add_argument(
    "--processes",
    type=int,
    default=1,
    help="Number of Pindos background processes to spawn",
)
parser.add_argument(
    "--kill",
    nargs="?",
    const="all",
    help="Kill 'all' tracked processes, or specify a specific PID to kill one",
)
...
if args.kill:
    if args.server_ip or args.server_port:
        parser.error(
            "Cannot mix process launching arguments (--server-ip/--server-port) with --kill."
        )
else:
...
if args.kill:
    ...
    if args.kill == "all":
        ...
        if args.kill not in tracked_pids:
            print(
                f"Warning: PID {args.kill} is not in the tracked process list. Attempting kill anyway..."
            )
    ...
    print(f"Launched {args.processes} Pindos background process(es).")
